...
## Approach
As mentioned above, we chose a deep Q learning as our approach for this project. Some other options which were considered included genetic algorithm or Q table based learning. The genetic algorithm, which was our initial proposal, was deemed to be unfeasible, due to the excessive amount of training time which would be required - each member of the population would need to fight in at least one 1v1 battle every generation, meaning dozens of battles would be required every generation, at a minimum. Then, hundreds, or possibly even thousands of generations would be needed to see significant results. A Q table based reinforcement learning algorithm would be another possible method, with advantages and disadvantages compared to our deep Q approach. Deep Q learning inherently works with our continuous state space, while with a Q table, we would have to discretize it, forcing us to make a tradeoff between the granularity of the state space and the size of our Q table. However, it is possible that, even with a large table size, a Q table may take fewer iterations to show favorable results, making it quicker to train overall. However, we decided on deep Q learning, as it seemed like the approach which best fit our problem.
...
When running an episode, actions are selected using a greedy epsilon algorithm, which either selects a random action with probability epsilon, or chooses the action which the neural network deems the highest quality given the current state. Epsilon is starts out high at the beginning of each training run, and slowly decays over the course of many episodes.

Our state space is relatively large, consisting of 26 variables. These include variables to indicate what the agent is currently doing (moving, strafing, blocking, turning, pitching, blocking, or drawing the bow), how long since it has attacked, how long it has been drawing the bow (if it is doing so), its remaining hitpoints, y position, and pitch, as well as all the above information about the opponent. Another important piece of information which the AI has access to is the forward/backward and left/right distance to the opponent. Finally, it knows the sine and cosine of the opponent's yaw angle relative to its frame of reference, which could allow the AI to determine which direction the opponent is looking. The reason for using the sine and cosine of the angle, rather than the angle itself, is that we have found that using a 360 degree angle variable as an input to a neural network does not work well, possibly because its value is modular, making gradient descent difficult.
...
## Evaluation
There are 2 primary metrics which we used for quantitative evaluation of our AIs' performance as they are training: average reward received, and damage done; both values are logged after every episode. Ideally both should show an increasing trend, as an increase in average reward indicates that the deep Q learning is functioning as intended, and that the AI is working to maximize its reward received, while damage done measures the agents' performance more directly, showing that they are succeeding in their primary goal. We trained our network in several sessions, each time carrying over the network from the previous session, so as to preserve some of the previous progress, but also re-raised the epsilon in order to allow for exploration. This approach is likely not optimal, but was used in order to perform tweaks such as modifying the reward function, without totally resetting our progress every time. Adjustment of the rewards was mostly based on qualitative evaluation of the agents' behavior, with the criteria that the agents should:
